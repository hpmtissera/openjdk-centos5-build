...
#wget -N http://ftp.tsukuba.wide.ad.jp/software/gcc/releases/gcc-4.8.2/gcc-4.8.2.tar.gz
...
docker run --volume "$(pwd):/opt/build" --cidfile=containerid  -u root hpmtissera/openjdk-centos5-build:1.0 sh -c "/opt/build/build-openjdk.sh"

CONTAINER_ID=$(cat containerid)
docker cp ${CONTAINER_ID}:/tmp/openjdk-11.0.4-linux-x64.tar.gz ./
